...
def test_inspection_endpoint_menolak_field_typo(client):
    ...
    typo nama field langsung 422, bukan diam-diam diabaikan (extra="allow"
    lama akan menelan typo tanpa peringatan)."""
    response = client.post(
        "/api/v1/inspections",
        json={"host_serial_code": "TIDAK-ADA", "field_yang_tidak_dikenal": "WO-123"},
    )
    assert response.status_code == 422
